...
This is a picture-matching/audio question. Audio prompt: "${r.audioPromptText}". Use these options: ${JSON.stringify(r.options)}`);const m=`You are generating Question #${o.slot} of 60 for the Cognix SEN Placement Assessment (aged 6-12).
...
}`;try{const p=await this.client.generateCompletion(m);if(p){let h=p.trim();h.startsWith("```json")?h=h.replace(/^```json/,"").replace(/```$/,"").trim():h.startsWith("```")&&(h=h.replace(/^```/,"").replace(/```$/,"").trim());const g=h.lastIndexOf("}");g!==-1&&(h=h.substring(0,g+1));const u=JSON.parse(h);if(u.instructions&&u.payload)return u.type=o.type,o.type!=="robot_mission"&&(delete u.payload.availableBlocks,delete u.payload.correctSequence),Array.isArray(u.payload.options)&&u.payload.options.length>3&&(u.payload.options=u.payload.options.slice(0,3)),o.type==="robot_mission"&&a&&(u.payload.availableBlocks=a.blocks,(!u.payload.correctSequence||u.payload.correctSequence.length===0)&&(u.payload.correctSequence=a.correctSequence)),o.type==="picture_match"&&r&&!u.payload.audioPromptText&&(u.payload.audioPromptText=r.audioPromptText),o.type==="picture_match"&&r&&(!Array.isArray(u.payload.options)||u.payload.options.length===0)&&(u.payload.options=r.options),{id:`q_slot_${o.slot}_${Date.now()}_${Math.floor(Math.random()*1e3)}`,slot:o.slot,domain:o.domain,skill:o.skill,subSkill:o.subSkill,title:u.title||o.title,instructions:u.instructions,difficulty:o.difficulty,expectedTimeMs:9e4,maxPoints:o.maxPoints,type:o.type,payload:u.payload,hintText:u.hintText||"Take your time and think carefully!"}}}catch{}return this.generateDynamicFallback(i)}generateDynamicFallback(e){const i=Math.max(1,Math.min(60,e)),o=U[i-1],a=`fallback_q_${i}_${Date.now()}_${Math.floor(Math.random()*1e3)}`;let r={},s=o.baselinePrompt,m="Look at all options carefully before picking.";if(o.type==="robot_mission"){const p=ue[i]||{blocks:["Move Forward ⬆️","Turn Right ➡️","Grab Item 🦾"],correctSequence:["Move Forward ⬆️","Grab Item 🦾"],description:"Build the correct sequence to complete the mission!"};s=p.description,r={availableBlocks:p.blocks,correctSequence:p.correctSequence,options:[{label:p.correctSequence.join(" ➔ "),correct:!0},{label:[...p.blocks].reverse().slice(0,2).join(" ➔ "),correct:!1},{label:p.blocks.slice(0,Math.min(2,p.blocks.length)).reverse().join(" ➔ "),correct:!1}],correctIndex:0},m=`Hint: ${p.correctSequence.join(" → then ")}`}else if(o.type==="picture_match"){const p=pe[i];p?(r={...p},s=p.audioPromptText,m="Listen carefully and tap the right picture!"):(r={audioPromptText:o.baselinePrompt,options:[{label:"Robot",emoji:"🤖",correct:!0},{label:"Apple",emoji:"🍎",correct:!1},{label:"Ball",emoji:"⚽",correct:!1}],correctIndex:0},m="Tap the correct picture!")}else if(o.type==="motor_target")r={targetsCount:{51:4,52:3,54:5,56:3}[i]||3,options:[{label:"Hit target 🎯",correct:!0},{label:"Miss edge",correct:!1},{label:"Click outside",correct:!1}],correctIndex:0},m="Click directly inside the glowing circle!";else{const p=[{sequence:["🔵","🔴","🔵","🔴","❓"],options:[{label:"🔵 Blue",correct:!0},{label:"🟢 Green",correct:!1},{label:"🟡 Yellow",correct:!1}],hint:"Blue and Red take turns!"},{sequence:["⭐","⭐","🌙","⭐","⭐","❓"],options:[{label:"🌙 Moon",correct:!0},{label:"⭐ Star",correct:!1},{label:"☀️ Sun",correct:!1}],hint:"Every third shape is a moon!"},{sequence:["🔺","🔶","🔺","🔶","❓"],options:[{label:"🔺 Triangle",correct:!0},{label:"⬛ Square",correct:!1},{label:"🔵 Circle",correct:!1}],hint:"Triangle and Diamond take turns!"},{sequence:["1️⃣","2️⃣","3️⃣","4️⃣","❓"],options:[{label:"5️⃣ Five",correct:!0},{label:"6️⃣ Six",correct:!1},{label:"3️⃣ Three",correct:!1}],hint:"Numbers go up by 1 each time!"},{sequence:["🐶","🐱","🐶","🐱","❓"],options:[{label:"🐶 Dog",correct:!0},{label:"🐸 Frog",correct:!1},{label:"🐱 Cat",correct:!1}],hint:"Dog and Cat take turns!"}],h=[{options:[{label:"😌 Stay calm and keep trying",correct:!0},{label:"😤 Get upset and quit",correct:!1},{label:"🚪 Leave the room",correct:!1}],hint:"Choose the patient option!"},{options:[{label:"🙋 Raise my hand politely",correct:!0},{label:"😴 Give up quietly",correct:!1},{label:"🗣️ Call out loudly",correct:!1}],hint:"Polite asking is best!"},{options:[{label:"⏸️ Stop and listen to teacher",correct:!0},{label:"🏃 Keep going",correct:!1},{label:"😶 Ignore instruction",correct:!1}],hint:"Always stop when teacher says STOP!"}],g=[{options:[{label:"⬅️ Left Arrow key",correct:!0},{label:"⬆️ Up Arrow key",correct:!1},{label:"➡️ Right Arrow key",correct:!1}],hint:"Left arrow moves things left!"},{options:[{label:"🔄 Restart the device",correct:!0},{label:"📵 Throw tablet away",correct:!1},{label:"😴 Wait forever",correct:!1}],hint:"Restarting fixes most freezes!"},{options:[{label:"✅ Place in matching slot",correct:!0},{label:"❌ Drop it anywhere",correct:!1},{label:"🔄 Spin it around",correct:!1}],hint:"Match the shape to the correct slot!"}];if(o.domain==="cognitive_ability"){const u=p[(i-1)%p.length];r={sequence:u.sequence,options:u.options,correctIndex:0},m=u.hint}else if(o.domain==="behavioral_readiness"){const u=h[Math.max(0,i-41)%h.length];r={options:u.options,correctIndex:0},m=u.hint}else if(o.domain==="fine_motor_technology"){const u=g[Math.max(0,i-53)%g.length];r={options:u.options,correctIndex:0},m=u.hint}else r={options:[{label:"🌟 Correct Solution",correct:!0},{label:"❌ Incorrect Action A",correct:!1},{label:"🛑 Incorrect Action B",correct:!1}],correctIndex:0}}if(r.options&&Array.isArray(r.options)&&r.options.length===3){const p=r.options.find(g=>g.correct)||r.options[0],h=this.shuffleArray([...r.options]);r.options=h,r.correctIndex=h.indexOf(p)}return{id:a,slot:o.slot,domain:o.domain,skill:o.skill,subSkill:o.subSkill,title:o.title,instructions:s,difficulty:o.difficulty,expectedTimeMs:9e4,maxPoints:o.maxPoints,type:o.type,payload:r,hintText:m}}shuffleArray(e){const i=[...e];for(let o=i.length-1;o>0;o--){const a=Math.floor(Math.random()*(o+1));[i[o],i[a]]=[i[a],i[o]]}return i}}const ee={cognitive_ability:{name:"Cognitive Ability",weight:.25,maxScore:25,questionCount:15,recommendedTimeMin:20},functional_skills:{name:"Functional Abilities",weight:.25,maxScore:25,questionCount:15,recommendedTimeMin:20},communication_level:{name:"Communication Level",weight:.2,maxScore:20,questionCount:10,recommendedTimeMin:15},behavioral_readiness:{name:"Behavioral & Learning Readiness",weight:.15,maxScore:15,questionCount:10,recommendedTimeMin:15},fine_motor_technology:{name:"Fine Motor & Technology Skills",weight:.15,maxScore:15,questionCount:10,recommendedTimeMin:20}};class oe{static calculateItemScore(e,i=2){if(!e.is_correct&&e.accuracy_score===0)return 0;const o=Math.max(0,Math.min(1,e.accuracy_score)),a=e.expected_time_ms||9e4,r=Math.max(0,(e.response_time_ms-a)/Math.max(1e3,a)),s=Math.max(.7,1-.1*r),m=Math.max(.5,1-.15*e.hints_used),p=Math.max(.6,1-.1*Math.max(0,e.attempts_count-1)),h=i*o*s*m*p;return Math.max(0,Math.min(i,Math.round(h*10)/10))}static calculateDomainScores(e,i){const o=["cognitive_ability","functional_skills","communication_level","behavioral_readiness","fine_motor_technology"],a={};for(const r of o){const s=ee[r],m=e.filter(f=>f.domain===r);if(m.length===0){a[r]={domain:r,domain_name:s.name,weight_pct:s.weight*100,max_score:s.maxScore,raw_accuracy_pct:0,efficiency_index:0,earned_score:0,skills_breakdown:{}};continue}let p=0,h=0;const g={};for(const f of m){const l=(i==null?void 0:i[f.item_id])??2,d=this.calculateItemScore(f,l);p+=d,h+=f.accuracy_score,g[f.skill]||(g[f.skill]={totalEarnedRatio:0,count:0}),g[f.skill].totalEarnedRatio+=d/Math.max(.1,l),g[f.skill].count+=1}const u=m.length,S=h/u*100,c=Math.min(s.maxScore,Math.round(p*10)/10),w={};for(const[f,l]of Object.entries(g))w[f]=Math.round(l.totalEarnedRatio/l.count*100);a[r]={domain:r,domain_name:s.name,weight_pct:s.weight*100,max_score:s.maxScore,raw_accuracy_pct:Math.round(S),efficiency_index:Math.round(c/s.maxScore*100)/100,earned_score:c,skills_breakdown:w}}return a}static calculateTotalScore(e){let i=0;for(const o of Object.values(e))i+=o.earned_score;return Math.min(100,Math.round(i*10)/10)}}class Ce{static evaluatePlacement(e,i,o){var F,D,b,C,N;let a="Explorer";e>=90?a="Innovator":e>=75?a="Creator":e>=60?a="Builder":a="Explorer";const r=[];(((F=i.cognitive_ability)==null?void 0:F.earned_score)||0)<10&&r.push({id:"FLAG_COGNITIVE_DEFICIENCY",type:"critical",title:"Cognitive Foundation Support",description:"Student demonstrated difficulty in pattern recognition and logical reasoning. Targeted logic puzzles recommended before advancing."}),(((D=i.functional_skills)==null?void 0:D.earned_score)||0)<10&&r.push({id:"FLAG_FUNCTIONAL_DEFICIENCY",type:"critical",title:"Multi-Step Mission Support",description:"Student requires scaffolded instruction following and working memory exercises."}),(((b=i.communication_level)==null?void 0:b.earned_score)||0)<7&&r.push({id:"FLAG_COMMUNICATION_SUPPORT",type:"warning",title:"Verbal & Visual Comprehension Support",description:"Audio visual cues and simplified instructions recommended during missions."}),(((C=i.behavioral_readiness)==null?void 0:C.earned_score)||0)<5.25&&r.push({id:"FLAG_BEHAVIORAL_ADAPTABILITY",type:"warning",title:"Error Recovery & Resilience Support",description:"Student showed hesitation or frustration during unexpected rule changes. Guided error-recovery feedback advised."}),(((N=i.fine_motor_technology)==null?void 0:N.earned_score)||0)<5.25&&r.push({id:"FLAG_FINE_MOTOR_SUPPORT",type:"info",title:"Digital Navigation Practice",description:"Drag-and-drop and target precision practice recommended for smooth touch/mouse control."});const u=r.some(q=>q.type==="critical");let S=a;u&&a!=="Explorer"&&(S=`${a} (with Targeted Support)`);let c=o.length||1,w=o.filter(q=>q.is_correct).length,f=o.reduce((q,j)=>q+j.hints_used,0),l=o.reduce((q,j)=>q+j.response_time_ms/Math.max(1e3,j.expected_time_ms),0)/c;const d=o.filter(q=>q.domain==="behavioral_readiness");let _=.75;if(d.length>0){const q=d.reduce((j,Y)=>j+Y.accuracy_score,0)/d.length;_=Math.round(q*100)/100}let E="Steady";if(o.length>=4){const q=Math.floor(o.length/2),j=o.slice(0,q).reduce((J,Z)=>J+Z.accuracy_score,0)/q,Y=o.slice(q).reduce((J,Z)=>J+Z.accuracy_score,0)/(o.length-q);Y-j>.15?E="High":Y<.4&&(E="Needs Practice")}return{totalScore:e,baseTrack:a,recommendedTrack:S,flags:r,requiresSupport:u,performanceIndicators:{overallAccuracy:Math.round(w/c*100),avgResponseSpeedRatio:Math.round(l*100)/100,hintDependencyRatio:Math.round(f/c*100)/100,adaptabilityIndex:_,learningProgressVelocity:E}}}}class Ee{constructor(){this.client=new ge}async generateReportSummary(e,i){var s;const o=Object.values(e.domain_scores||{}).map(m=>`${m.domain_name}: ${m.earned_score}/${m.max_score} (${Math.round(m.earned_score/m.max_score*100)}%)`).join(", "),a=`Analyze this student assessment telemetry and provide a detailed 4-paragraph diagnostic report:
...
Domain Scores: ${o}
Timeouts Count: ${((s=e.question_time_records)==null?void 0:s.filter(m=>m==null?void 0:m.timedOut).length)||0}
Total Active Time: ${Math.round((e.total_active_duration_ms||0)/6e4)} mins
Flags: ${i.flags.map(m=>`${m.title}: ${m.description}`).join(" | ")||"None"}
...
4. Specific SEN Accommodations & Action Plan for Educator/Parent.`,r=await this.client.generateCompletion(a,"You are an expert educational psychologist and SEN assessment specialist. Provide detailed, compassionate, highly specific diagnostic reports.");return r&&r.length>200?r:this.getBuiltInReport(e,i)}getBuiltInReport(e,i){const{totalScore:o,recommendedTrack:a,performanceIndicators:r,flags:s}=i,p=[...Object.values(e.domain_scores||{})].sort((d,_)=>{const E=d.max_score>0?d.earned_score/d.max_score:0;return(_.max_score>0?_.earned_score/_.max_score:0)-E}),h=p[0],g=p[p.length-1],u=h&&h.max_score>0?Math.round(h.earned_score/h.max_score*100):0,S=g&&g.max_score>0?Math.round(g.earned_score/g.max_score*100):0,c=e.question_time_records||[],w=c.filter(d=>d==null?void 0:d.timedOut).length,f=c.length>0?(c.reduce((d,_)=>d+((_==null?void 0:_.responseLatencyMs)||0),0)/c.length/1e3).toFixed(1):"0";let l=`### Executive Diagnostic Summary
...
`,l+=`- **Primary Strength**: **${(h==null?void 0:h.domain_name)||"Cognitive Skills"}** (${u}% mastery). Demonstrates confident grasp of these core concepts.
`,l+=`- **Primary Growth Area**: **${(g==null?void 0:g.domain_name)||"Fine Motor"}** (${S}% mastery). Benefits from targeted support and scaffolded practice in this area.
...
`,S<60?(l+=`1. **Scaffolded Learning**: Break complex multi-step instructions into single 1-step visual prompts.
...
`,s.length>0?l+=`
> [!WARNING]
> **Identified Support Flags**: ${s.map(d=>d.title).join(" • ")}.`:l+=`
> [!TIP]
> **Exceptional Performance**: Student displayed balanced competence across all 5 evaluation domains.`,l}}var re={};(function k(e,i,o,a){var r=!!(e.Worker&&e.Blob&&e.Promise&&e.OffscreenCanvas&&e.OffscreenCanvasRenderingContext2D&&e.HTMLCanvasElement&&e.HTMLCanvasElement.prototype.transferControlToOffscreen&&e.URL&&e.URL.createObjectURL),s=typeof Path2D=="function"&&typeof DOMMatrix=="function",m=(function(){if(!e.OffscreenCanvas)return!1;try{var n=new OffscreenCanvas(1,1),t=n.getContext("2d");t.fillRect(0,0,1,1);var y=n.transferToImageBitmap();t.createPattern(y,"no-repeat")}catch{return!1}return!0})();function p(){}function h(n){var t=i.exports.Promise,y=t!==void 0?t:e.Promise;return typeof y=="function"?new y(n):(n(p,p),null)}var g=(function(n,t){return{transform:function(y){if(n)return y;if(t.has(y))return t.get(y);var x=new OffscreenCanvas(y.width,y.height),T=x.getContext("2d");return T.drawImage(y,0,0),t.set(y,x),x},clear:function(){t.clear()}}})(m,new Map),u=(function(){var n=Math.floor(16.666666666666668),t,y,x={},T=0;return typeof requestAnimationFrame=="function"&&typeof cancelAnimationFrame=="function"?(t=function(M){var P=Math.random();return x[P]=requestAnimationFrame(function v(A){T===A||T+n-1<A?(T=A,delete x[P],M()):x[P]=requestAnimationFrame(v)}),P},y=function(M){x[M]&&cancelAnimationFrame(x[M])}):(t=function(M){return setTimeout(M,n)},y=function(M){return clearTimeout(M)}),{frame:t,cancel:y}})(),S=(function(){var n,t,y={};function x(T){function M(P,v){T.postMessage({options:P||{},callback:v})}T.init=function(v){var A=v.transferControlToOffscreen();T.postMessage({canvas:A},[A])},T.fire=function(v,A,$){if(t)return M(v,null),t;var O=Math.random().toString(36).slice(2);return t=h(function(R){function L(B){B.data.callback===O&&(delete y[O],T.removeEventListener("message",L),t=null,g.clear(),$(),R())}T.addEventListener("message",L),M(v,O),y[O]=L.bind(null,{data:{callback:O}})}),t},T.reset=function(){T.postMessage({reset:!0});for(var v in y)y[v](),delete y[v]}}return function(){if(n)return n;if(!o&&r){var T=["var CONFETTI, SIZE = {}, module = {};","("+k.toString()+")(this, module, true, SIZE);","onmessage = function(msg) {","  if (msg.data.options) {","    CONFETTI(msg.data.options).then(function () {","      if (msg.data.callback) {","        postMessage({ callback: msg.data.callback });","      }","    });","  } else if (msg.data.reset) {","    CONFETTI && CONFETTI.reset();","  } else if (msg.data.resize) {","    SIZE.width = msg.data.resize.width;","    SIZE.height = msg.data.resize.height;","  } else if (msg.data.canvas) {","    SIZE.width = msg.data.canvas.width;","    SIZE.height = msg.data.canvas.height;","    CONFETTI = module.exports.create(msg.data.canvas);","  }","}"].join(`
`);try{n=new Worker(URL.createObjectURL(new Blob([T])))}catch(M){return typeof console<"u"&&typeof console.warn=="function"&&console.warn("🎊 Could not load worker",M),null}x(n)}return n}})(),c={particleCount:50,angle:90,spread:45,startVelocity:45,decay:.9,gravity:1,drift:0,ticks:200,x:.5,y:.5,shapes:["square","circle"],zIndex:100,colors:["#26ccff","#a25afd","#ff5e7e","#88ff5a","#fcff42","#ffa62d","#ff36ff"],disableForReducedMotion:!1,scalar:1};function w(n,t){return t?t(n):n}function f(n){return n!=null}function l(n,t,y){return w(n&&f(n[t])?n[t]:c[t],y)}function d(n){return n<0?0:Math.floor(n)}function _(n,t){return Math.floor(Math.random()*(t-n))+n}function E(n){return parseInt(n,16)}function F(n){return n.map(D)}function D(n){var t=String(n).replace(/[^0-9a-f]/gi,"");return t.length<6&&(t=t[0]+t[0]+t[1]+t[1]+t[2]+t[2]),{r:E(t.substring(0,2)),g:E(t.substring(2,4)),b:E(t.substring(4,6))}}function b(n){var t=l(n,"origin",Object);return t.x=l(t,"x",Number),t.y=l(t,"y",Number),t}function C(n){n.width=document.documentElement.clientWidth,n.height=document.documentElement.clientHeight}function N(n){var t=n.getBoundingClientRect();n.width=t.width,n.height=t.height}function q(n){var t=document.createElement("canvas");return t.style.position="fixed",t.style.top="0px",t.style.left="0px",t.style.pointerEvents="none",t.style.zIndex=n,t}function j(n,t,y,x,T,M,P,v,A){n.save(),n.translate(t,y),n.rotate(M),n.scale(x,T),n.arc(0,0,1,P,v,A),n.restore()}function Y(n){var t=n.angle*(Math.PI/180),y=n.spread*(Math.PI/180);return{x:n.x,y:n.y,wobble:Math.random()*10,wobbleSpeed:Math.min(.11,Math.random()*.1+.05),velocity:n.startVelocity*.5+Math.random()*n.startVelocity,angle2D:-t+(.5*y-Math.random()*y),tiltAngle:(Math.random()*(.75-.25)+.25)*Math.PI,color:n.color,shape:n.shape,tick:0,totalTicks:n.ticks,decay:n.decay,drift:n.drift,random:Math.random()+2,tiltSin:0,tiltCos:0,wobbleX:0,wobbleY:0,gravity:n.gravity*3,ovalScalar:.6,scalar:n.scalar,flat:n.flat}}function J(n,t){t.x+=Math.cos(t.angle2D)*t.velocity+t.drift,t.y+=Math.sin(t.angle2D)*t.velocity+t.gravity,t.velocity*=t.decay,t.flat?(t.wobble=0,t.wobbleX=t.x+10*t.scalar,t.wobbleY=t.y+10*t.scalar,t.tiltSin=0,t.tiltCos=0,t.random=1):(t.wobble+=t.wobbleSpeed,t.wobbleX=t.x+10*t.scalar*Math.cos(t.wobble),t.wobbleY=t.y+10*t.scalar*Math.sin(t.wobble),t.tiltAngle+=.1,t.tiltSin=Math.sin(t.tiltAngle),t.tiltCos=Math.cos(t.tiltAngle),t.random=Math.random()+2);var y=t.tick++/t.totalTicks,x=t.x+t.random*t.tiltCos,T=t.y+t.random*t.tiltSin,M=t.wobbleX+t.random*t.tiltCos,P=t.wobbleY+t.random*t.tiltSin;if(n.fillStyle="rgba("+t.color.r+", "+t.color.g+", "+t.color.b+", "+(1-y)+")",n.beginPath(),s&&t.shape.type==="path"&&typeof t.shape.path=="string"&&Array.isArray(t.shape.matrix))n.fill(ye(t.shape.path,t.shape.matrix,t.x,t.y,Math.abs(M-x)*.1,Math.abs(P-T)*.1,Math.PI/10*t.wobble));else if(t.shape.type==="bitmap"){var v=Math.PI/10*t.wobble,A=Math.abs(M-x)*.1,$=Math.abs(P-T)*.1,O=t.shape.bitmap.width*t.scalar,R=t.shape.bitmap.height*t.scalar,L=new DOMMatrix([Math.cos(v)*A,Math.sin(v)*A,-Math.sin(v)*$,Math.cos(v)*$,t.x,t.y]);L.multiplySelf(new DOMMatrix(t.shape.matrix));var B=n.createPattern(g.transform(t.shape.bitmap),"no-repeat");B.setTransform(L),n.globalAlpha=1-y,n.fillStyle=B,n.fillRect(t.x-O/2,t.y-R/2,O,R),n.globalAlpha=1}else if(t.shape==="circle")n.ellipse?n.ellipse(t.x,t.y,Math.abs(M-x)*t.ovalScalar,Math.abs(P-T)*t.ovalScalar,Math.PI/10*t.wobble,0,2*Math.PI):j(n,t.x,t.y,Math.abs(M-x)*t.ovalScalar,Math.abs(P-T)*t.ovalScalar,Math.PI/10*t.wobble,0,2*Math.PI);else if(t.shape==="star")for(var I=Math.PI/2*3,Q=4*t.scalar,G=8*t.scalar,H=t.x,V=t.y,K=5,W=Math.PI/K;K--;)H=t.x+Math.cos(I)*G,V=t.y+Math.sin(I)*G,n.lineTo(H,V),I+=W,H=t.x+Math.cos(I)*Q,V=t.y+Math.sin(I)*Q,n.lineTo(H,V),I+=W;else n.moveTo(Math.floor(t.x),Math.floor(t.y)),n.lineTo(Math.floor(t.wobbleX),Math.floor(T)),n.lineTo(Math.floor(M),Math.floor(P)),n.lineTo(Math.floor(x),Math.floor(t.wobbleY));return n.closePath(),n.fill(),t.tick<t.totalTicks}function Z(n,t,y,x,T){var M=t.slice(),P=n.getContext("2d"),v,A,$=h(function(O){function R(){v=A=null,P.clearRect(0,0,x.width,x.height),g.clear(),T(),O()}function L(){o&&!(x.width===a.width&&x.height===a.height)&&(x.width=n.width=a.width,x.height=n.height=a.height),!x.width&&!x.height&&(y(n),x.width=n.width,x.height=n.height),P.clearRect(0,0,x.width,x.height),M=M.filter(function(B){return J(P,B)}),M.length?v=u.frame(L):R()}v=u.frame(L),A=R});return{addFettis:function(O){return M=M.concat(O),$},canvas:n,promise:$,reset:function(){v&&u.cancel(v),A&&A()}}}function ae(n,t){var y=!n,x=!!l(t||{},"resize"),T=!1,M=l(t,"disableForReducedMotion",Boolean),P=r&&!!l(t||{},"useWorker"),v=P?S():null,A=y?C:N,$=n&&v?!!n.__confetti_initialized:!1,O=typeof matchMedia=="function"&&matchMedia("(prefers-reduced-motion)").matches,R;function L(I,Q,G){for(var H=l(I,"particleCount",d),V=l(I,"angle",Number),K=l(I,"spread",Number),W=l(I,"startVelocity",Number),Se=l(I,"decay",Number),ke=l(I,"gravity",Number),we=l(I,"drift",Number),le=l(I,"colors",F),_e=l(I,"ticks",Number),ce=l(I,"shapes"),Te=l(I,"scalar"),Me=!!l(I,"flat"),de=b(I),me=H,ie=[],Pe=n.width*de.x,Ae=n.height*de.y;me--;)ie.push(Y({x:Pe,y:Ae,angle:V,spread:K,startVelocity:W,color:le[me%le.length],shape:ce[_(0,ce.length)],ticks:_e,decay:Se,gravity:ke,drift:we,scalar:Te,flat:Me}));return R?R.addFettis(ie):(R=Z(n,ie,A,Q,G),R.promise)}function B(I){var Q=M||l(I,"disableForReducedMotion",Boolean),G=l(I,"zIndex",Number);if(Q&&O)return h(function(W){W()});y&&R?n=R.canvas:y&&!n&&(n=q(G),document.body.appendChild(n)),x&&!$&&A(n);var H={width:n.width,height:n.height};v&&!$&&v.init(n),$=!0,v&&(n.__confetti_initialized=!0);function V(){if(v){var W={getBoundingClientRect:function(){if(!y)return n.getBoundingClientRect()}};A(W),v.postMessage({resize:{width:W.width,height:W.height}});return}H.width=H.height=null}function K(){R=null,x&&(T=!1,e.removeEventListener("resize",V)),y&&n&&(document.body.contains(n)&&document.body.removeChild(n),n=null,$=!1)}return x&&!T&&(T=!0,e.addEventListener("resize",V,!1)),v?v.fire(I,H,K):L(I,H,K)}return B.reset=function(){v&&v.reset(),R&&R.reset()},B}var te;function se(){return te||(te=ae(null,{useWorker:!0,resize:!0})),te}function ye(n,t,y,x,T,M,P){var v=new Path2D(n),A=new Path2D;A.addPath(v,new DOMMatrix(t));var $=new Path2D;return $.addPath(A,new DOMMatrix([Math.cos(P)*T,Math.sin(P)*T,-Math.sin(P)*M,Math.cos(P)*M,y,x])),$}function ve(n){if(!s)throw new Error("path confetti are not supported in this browser");var t,y;typeof n=="string"?t=n:(t=n.path,y=n.matrix);var x=new Path2D(t),T=document.createElement("canvas"),M=T.getContext("2d");if(!y){for(var P=1e3,v=P,A=P,$=0,O=0,R,L,B=0;B<P;B+=2)for(var I=0;I<P;I+=2)M.isPointInPath(x,B,I,"nonzero")&&(v=Math.min(v,B),A=Math.min(A,I),$=Math.max($,B),O=Math.max(O,I));R=$-v,L=O-A;var Q=10,G=Math.min(Q/R,Q/L);y=[G,0,0,G,-Math.round(R/2+v)*G,-Math.round(L/2+A)*G]}return{type:"path",path:t,matrix:y}}function xe(n){var t,y=1,x="#000000",T='"Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji", "EmojiOne Color", "Android Emoji", "Twemoji Mozilla", "system emoji", sans-serif';typeof n=="string"?t=n:(t=n.text,y="scalar"in n?n.scalar:y,T="fontFamily"in n?n.fontFamily:T,x="color"in n?n.color:x);var M=10*y,P=""+M+"px "+T,v=new OffscreenCanvas(M,M),A=v.getContext("2d");A.font=P;var $=A.measureText(t),O=Math.ceil($.actualBoundingBoxRight+$.actualBoundingBoxLeft),R=Math.ceil($.actualBoundingBoxAscent+$.actualBoundingBoxDescent),L=2,B=$.actualBoundingBoxLeft+L,I=$.actualBoundingBoxAscent+L;O+=L+L,R+=L+L,v=new OffscreenCanvas(O,R),A=v.getContext("2d"),A.font=P,A.fillStyle=x,A.fillText(t,B,I);var Q=1/y;return{type:"bitmap",bitmap:v.transferToImageBitmap(),matrix:[Q,0,0,Q,-O*Q/2,-R*Q/2]}}i.exports=function(){return se().apply(this,arguments)},i.exports.reset=function(){se().reset()},i.exports.create=ae,i.exports.shapeFromPath=ve,i.exports.shapeFromText=xe})((function(){return typeof window<"u"?window:typeof self<"u"?self:this||{}})(),re,!1);const $e=re.exports;re.exports.create;function fe(k,e,i){try{$e({particleCount:100,spread:70,origin:{y:.6}})}catch{}const{totalScore:o,recommendedTrack:a,flags:r,performanceIndicators:s}=i;let m="";r.length>0&&(m=r.map(b=>`
    ...
    `).join(""));const p=Object.values(e.domain_scores).map(b=>{const C=Math.round(b.earned_score/b.max_score*100);return`
    ...
    `}).join(""),h=e.question_time_records||[],g=h.map(b=>{if(!b)return"";const C=Math.round(b.activeDurationMs/1e3),N=b.responseLatencyMs?(b.responseLatencyMs/1e3).toFixed(1)+"s":"—",q=b.remainingTimeWhenAnsweredMs?Math.round(b.remainingTimeWhenAnsweredMs/1e3)+"s":"0s";let j='<span style="color:#10b981; font-weight:700;">🟢 Fast</span>';return b.timedOut?j='<span style="color:#ef4444; font-weight:700;">⏰ Timed Out</span>':C>80?j='<span style="color:#f59e0b; font-weight:700;">🔴 Slow</span>':C>45&&(j='<span style="color:#3b82f6; font-weight:700;">🟡 Normal</span>'),`
      <tr style="${b.timedOut?"background:rgba(239,68,68,0.08);":""} border-bottom:1px solid rgba(255,255,255,0.05); font-size:0.85rem;">
        ...
        <td style="padding:0.6rem 0.8rem; text-align:center;">${j}</td>
    ...
    `}).join(""),u=e.break_events||[];let S="";u.length>0?S=u.map(b=>{const C=Math.floor(b.breakDurationMs/6e4),N=Math.round(b.breakDurationMs%6e4/1e3),q=Math.round(b.countdownRemainingAtPause);return`
        <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.04); border:1px solid var(--border-color); padding:0.75rem 1rem; border-radius:10px; margin-bottom:0.5rem; font-size:0.88rem;">
          ...
          <div style="color:var(--accent-amber); font-weight:700;">
            Duration: ${C>0?`${C}m `:""}${N}s (Timer left: ${q}s)
          </div>
        </div>
      `}).join(""):S=`
    ...
    `;const c=Math.round((e.total_active_duration_ms||0)/6e4),w=Math.round((e.total_break_duration_ms||0)/6e4),f=Math.round((e.total_wall_clock_duration_ms||0)/6e4),l=h.filter(b=>b==null?void 0:b.timedOut).length;Re(e),k.innerHTML=`
  ...
  `;const d=k.querySelector("#restart-btn");d&&d.addEventListener("click",()=>{window.location.reload()});const _=k.querySelector("#print-report-btn");_&&_.addEventListener("click",()=>window.print());const E=k.querySelector("#download-pdf-btn");E&&E.addEventListener("click",()=>Oe(e));const F=k.querySelector("#download-csv-btn");F&&F.addEventListener("click",()=>{Le(e)});const D=k.querySelector("#ceo-dashboard-btn");D&&D.addEventListener("click",()=>{be(k)})}function Re(k){try{const e=localStorage.getItem("cognix_all_sessions");let i=e?JSON.parse(e):[];i=i.filter(o=>o.session_id!==k.session_id),i.unshift(k),localStorage.setItem("cognix_all_sessions",JSON.stringify(i))}catch{}}function be(k){const e=localStorage.getItem("cognix_all_sessions"),i=e?JSON.parse(e):[];if(i.length===0){k.innerHTML=`
    ...
    `;const c=k.querySelector("#ceo-new-test-btn");c&&c.addEventListener("click",()=>window.location.reload());return}const o=i.length,a=Math.round(i.reduce((c,w)=>c+(w.total_score||0),0)/o),r=(i.reduce((c,w)=>c+(w.total_active_duration_ms||0)/6e4,0)/o).toFixed(1),s=i.filter(c=>Array.isArray(c.flags)&&c.flags.length>0).length,p=[{key:"cognitive_ability",name:"Cognitive Ability"},{key:"functional_skills",name:"Functional Skills"},{key:"communication_level",name:"Communication Level"},{key:"behavioral_readiness",name:"Behavioral Readiness"},{key:"fine_motor_technology",name:"Fine Motor & Tech"}].map(c=>{let w=0,f=0;i.forEach(d=>{if(d.domain_scores&&d.domain_scores[c.key]){const _=d.domain_scores[c.key];w+=_.earned_score,f+=_.max_score}});const l=f>0?Math.round(w/f*100):0;return{name:c.name,pct:l}});let h=i.map((c,w)=>{const f=Math.round((c.total_active_duration_ms||0)/6e4),l=Array.isArray(c.flags)?c.flags.length:0,d=c.start_time?new Date(c.start_time).toLocaleDateString():"Today";return`
      <tr style="border-bottom: 1px solid rgba(255,255,255,0.05); font-size: 0.88rem;">
        ...
        <td style="padding: 0.75rem 1rem; color: var(--text-secondary); font-size: 0.8rem;">${d}</td>
  ...
  `;const g=k.querySelector("#export-all-csv-btn");g&&g.addEventListener("click",()=>qe(i));const u=k.querySelector("#ceo-restart-btn");u&&u.addEventListener("click",()=>window.location.reload()),k.querySelectorAll(".view-session-btn").forEach(c=>{c.addEventListener("click",w=>{const l=w.currentTarget.getAttribute("data-id"),d=i.find(_=>_.session_id===l);if(d){const _={totalScore:d.total_score,placedTrack:d.placed_track||"Level 1",recommendedTrack:d.recommended_track||"Level 1",flags:Array.isArray(d.flags)?d.flags.map(E=>typeof E=="string"?{title:E,description:"",type:"advisory"}:E):[],performanceIndicators:{overallAccuracy:d.total_score,adaptabilityIndex:.85,learningProgressVelocity:"Steady",hintDependencyRatio:.1}};fe(k,d,_)}})})}function qe(k){let e=`StudentName,AgeGroup,TotalScore,PlacedTrack,ActiveTimeMins,Timeouts,BreaksCount,FlagsCount,CompletedDate
`;k.forEach(r=>{var u,S;const s=((r.total_active_duration_ms||0)/6e4).toFixed(1),m=((u=r.question_time_records)==null?void 0:u.filter(c=>c==null?void 0:c.timedOut).length)||0,p=((S=r.break_events)==null?void 0:S.length)||0,h=Array.isArray(r.flags)?r.flags.length:0,g=r.start_time?new Date(r.start_time).toLocaleDateString():"Today";e+=`"${r.student_name.replace(/"/g,'""')}","${r.age_group||"7-9"}",${r.total_score},"${r.placed_track||r.recommended_track||"Level 1"}",${s},${m},${p},${h},"${g}"
...
`;e.forEach(s=>{if(!s)return;const m=(s.activeDurationMs/1e3).toFixed(1),p=s.responseLatencyMs?(s.responseLatencyMs/1e3).toFixed(1):"",h=s.remainingTimeWhenAnsweredMs?(s.remainingTimeWhenAnsweredMs/1e3).toFixed(1):"0",g=s.timedOut?"TIMED_OUT":s.activeDurationMs>8e4?"SLOW":s.activeDurationMs>45e3?"NORMAL":"FAST";i+=`${s.questionSlot},"${s.domain}","${s.subSkill}","${s.questionTitle.replace(/"/g,'""')}",${m},${p},${h},${g},${s.timedOut},${s.breaksDuringQuestion},${s.earnedScore},${s.maxScore}
`});const o=new Blob([i],{type:"text/csv;charset=utf-8;"}),a=URL.createObjectURL(o),r=document.createElement("a");r.setAttribute("href",a),r.setAttribute("download",`Cognix_CEO_Assessment_Time_Report_${k.student_name.replace(/\s+/g,"_")}.csv`),r.click()}function De(k){return k.replace(/^### (.*$)/gim,'<h3 style="color:var(--text-primary); font-size:1.05rem; margin-top:1rem; margin-bottom:0.4rem;">$1</h3>').replace(/\*\*(.*?)\*\*/g,"<strong>$1</strong>").replace(/^> (.*$)/gim,'<blockquote style="border-left:3px solid var(--accent-cyan); padding-left:0.8rem; margin:0.8rem 0; color:var(--accent-cyan); font-size:0.9rem;">$1</blockquote>')}const z=class z{constructor(e){this.studentName="Alex Rivers",this.cachedActivities=new Array(60).fill(null),this.userAnswers=[],this.questionTimeRecords=[],this.breakEvents=[],this.currentQuestionIndex=0,this.totalTimerSeconds=0,this.questionTimerSecondsRemaining=z.QUESTION_TIME_LIMIT_SEC,this.itemStartTimestamp=0,this.itemFirstInteractionTimestamp=null,this.currentPauseStartTimestamp=null,this.pauseDurationForCurrentQuestionMs=0,this.isPaused=!1,this.globalTimerInterval=null,this.questionTimerInterval=null,this.loadGen=0,this.isLoadingNextQuestion=!1,this.motorTargetPos={top:80,left:240},this.beforeUnloadHandler=()=>{this.saveSession(this.studentName)},this.container=e,this.generator=new Ie,this.analyzer=new Ee,this.initUserAnswers()}static getSavedSession(){try{const e=localStorage.getItem(z.STORAGE_KEY);if(!e)return null;const i=JSON.parse(e);return i&&Array.isArray(i.userAnswers)&&i.userAnswers.length===60?i:null}catch{return null}}static clearSavedSession(){try{localStorage.removeItem(z.STORAGE_KEY)}catch{}}saveSession(e=this.studentName){try{this.studentName=e;const i={studentName:e,currentQuestionIndex:this.currentQuestionIndex,totalTimerSeconds:this.totalTimerSeconds,currentQuestionRemainingSeconds:this.questionTimerSecondsRemaining,isPaused:this.isPaused,cachedActivities:this.cachedActivities,userAnswers:this.userAnswers,questionTimeRecords:this.questionTimeRecords,breakEvents:this.breakEvents,savedAt:Date.now()};localStorage.setItem(z.STORAGE_KEY,JSON.stringify(i))}catch{}}attachBeforeUnload(){window.removeEventListener("beforeunload",this.beforeUnloadHandler),window.addEventListener("beforeunload",this.beforeUnloadHandler)}detachBeforeUnload(){window.removeEventListener("beforeunload",this.beforeUnloadHandler)}initUserAnswers(){this.userAnswers=new Array(60).fill(null).map(()=>({selectedAnswerIndex:null,robotSequence:[],motorClicks:[],attemptsCount:0,hintsUsed:0,timeSpentMs:0,isSolved:!1,timedOut:!1,answeredAt:null,responseLatencyMs:null,remainingTimeWhenAnsweredMs:null,breaksDuringQuestion:0})),this.questionTimeRecords=[],this.breakEvents=[]}async startSession(e="Alex Rivers",i=!0){this.studentName=e;const o=i?z.getSavedSession():null;o?(this.currentQuestionIndex=Math.max(0,Math.min(59,o.currentQuestionIndex||0)),this.cachedActivities=o.cachedActivities||new Array(60).fill(null),this.userAnswers=o.userAnswers,this.questionTimeRecords=o.questionTimeRecords||[],this.breakEvents=o.breakEvents||[],this.totalTimerSeconds=o.totalTimerSeconds||0,this.questionTimerSecondsRemaining=o.currentQuestionRemainingSeconds||z.QUESTION_TIME_LIMIT_SEC,this.isPaused=o.isPaused||!1,this.startGlobalTimer(this.totalTimerSeconds),this.attachBeforeUnload(),this.isPaused?(await this.loadQuestion(this.currentQuestionIndex,!1),this.pauseAssessment()):await this.loadQuestion(this.currentQuestionIndex,!1)):(this.currentQuestionIndex=0,this.cachedActivities=new Array(60).fill(null),this.initUserAnswers(),this.totalTimerSeconds=0,this.questionTimerSecondsRemaining=z.QUESTION_TIME_LIMIT_SEC,this.isPaused=!1,this.startGlobalTimer(0),this.attachBeforeUnload(),this.saveSession(e),await this.loadQuestion(0,!0))}startGlobalTimer(e=0){this.totalTimerSeconds=e,this.globalTimerInterval&&clearInterval(this.globalTimerInterval),this.globalTimerInterval=setInterval(()=>{if(!this.isPaused){this.totalTimerSeconds++,this.totalTimerSeconds%3===0&&this.saveSession(this.studentName);const i=document.getElementById("global-timer");if(i){const o=String(Math.floor(this.totalTimerSeconds/60)).padStart(2,"0"),a=String(this.totalTimerSeconds%60).padStart(2,"0");i.textContent=`${o}:${a}`}}},1e3)}speakAudio(e){try{if("speechSynthesis"in window){window.speechSynthesis.cancel();const i=new SpeechSynthesisUtterance(e);i.rate=.9,i.pitch=1,window.speechSynthesis.speak(i)}}catch{}}startQuestionTimer(){this.questionTimerInterval&&(clearInterval(this.questionTimerInterval),this.questionTimerInterval=null),this.questionTimerSecondsRemaining<=0&&(this.questionTimerSecondsRemaining=z.QUESTION_TIME_LIMIT_SEC),this.questionTimerInterval=setInterval(()=>{this.isPaused||(this.questionTimerSecondsRemaining--,this.updateQuestionTimerUI(),this.questionTimerSecondsRemaining<=0&&(clearInterval(this.questionTimerInterval),this.questionTimerInterval=null,this.handleQuestionTimeout()))},1e3)}updateQuestionTimerUI(){const e=document.getElementById("question-timer-display"),i=document.getElementById("question-timer-ring");if(e){const o=Math.floor(this.questionTimerSecondsRemaining/60),a=String(this.questionTimerSecondsRemaining%60).padStart(2,"0");e.textContent=`${o}:${a}`,this.questionTimerSecondsRemaining<=15?e.style.color="#ef4444":e.style.color="var(--accent-cyan)"}if(i){const o=this.questionTimerSecondsRemaining/z.QUESTION_TIME_LIMIT_SEC*100;i.style.width=`${o}%`}}async loadQuestion(e,i=!0){var s;if(e<0||e>=60)return;this.motorTargetPos={top:80,left:240};const o=++this.loadGen;this.currentQuestionIndex=e;const a=U[e];if(this.questionTimerInterval&&(clearInterval(this.questionTimerInterval),this.questionTimerInterval=null),(i||this.questionTimerSecondsRemaining<=3)&&(this.questionTimerSecondsRemaining=z.QUESTION_TIME_LIMIT_SEC),this.itemFirstInteractionTimestamp=null,this.pauseDurationForCurrentQuestionMs=0,!this.cachedActivities[e]&&(this.renderLoadingState(e),this.cachedActivities[e]=await this.generator.generateActivity(a.slot),o!==this.loadGen))return;this.isLoadingNextQuestion=!1,this.itemStartTimestamp=Date.now(),this.saveSession(this.studentName),this.render();const r=this.cachedActivities[e];if(r&&r.type==="picture_match"){const m=((s=r.payload)==null?void 0:s.audioPromptText)||r.instructions;m&&this.speakAudio(m)}this.isPaused||this.startQuestionTimer(),this.prefetchNextQuestion(e+1)}renderLoadingState(e){const i=U[e],o=ee[i.domain],a=this.container.querySelector("#playground-area");a&&(a.innerHTML=`
      ...
      `),setTimeout(i,2200)}render(){const e=this.cachedActivities[this.currentQuestionIndex];if(!e)return;const i=U[this.currentQuestionIndex],o=ee[i.domain],a=this.userAnswers[this.currentQuestionIndex],s=U[this.currentQuestionIndex].domain,m=U.filter(g=>g.domain===s),p=Array.from(new Set(m.map(g=>g.subSkill)));let h=`
      <div class="skill-ladder-bar" style="background: rgba(15,23,42,0.8); border: 1px solid var(--border-color); border-radius: 16px; padding: 1rem 1.25rem; margin-bottom: 1.25rem; backdrop-filter: blur(10px);">
        <div class="ladder-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.75rem; flex-wrap:wrap; gap:0.5rem;">
          <div style="display:flex; align-items:center; gap:0.6rem;">
            ...
            <div>
              ...
              <div style="font-size:0.8rem; color:var(--text-secondary);">
                Active Skill: <strong style="color: #fff;">${i.subSkill}</strong> (Q${i.slot} of 60)
              </div>
            </div>
          </div>
          <div style="font-size:0.85rem; font-weight:700; background:rgba(16,185,129,0.15); border:1px solid var(--accent-emerald); padding:0.35rem 0.85rem; border-radius:12px; color:var(--accent-emerald);">
            Progress: ${this.userAnswers.filter(g=>g.isSolved||g.timedOut).length} / 60 Complete
          </div>
        </div>
    ...
    `;p.forEach((g,u)=>{const S=m.filter(_=>_.subSkill===g),c=i.subSkill===g,w=S.filter(_=>{const E=this.userAnswers[_.slot-1];return E&&(E.isSolved||E.timedOut)}).length,f=w===S.length;let l=`${u+1}`,d="background: rgba(255,255,255,0.04); border: 1px solid var(--border-color); color: var(--text-secondary);";c?(l="🐸",d="background: linear-gradient(135deg, rgba(6,182,212,0.3), rgba(59,130,246,0.3)); border: 1px solid var(--accent-cyan); color: #fff; box-shadow: 0 0 12px rgba(6,182,212,0.3);"):f&&(l="⭐",d="background: rgba(16,185,129,0.15); border: 1px solid var(--accent-emerald); color: var(--accent-emerald);"),h+=`
        <div style="flex:1; min-width:135px; ${d} padding:0.5rem 0.75rem; border-radius:10px; display:flex; flex-direction:column; gap:0.3rem;">
          <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.8rem; font-weight:700;">
            <span style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:95px;" title="${g}">${g}</span>
            <span style="font-size:0.85rem;">${l}</span>
          </div>
          <div style="font-size:0.72rem; opacity:0.85; display:flex; justify-content:space-between;">
            <span>${w}/${S.length} Qs</span>
          ...
          <div style="height:4px; width:100%; background:rgba(255,255,255,0.1); border-radius:2px; overflow:hidden;">
            <div style="width:${w/S.length*100}%; height:100%; background:${f?"var(--accent-emerald)":"var(--accent-cyan)"}; transition:width 0.3s ease;"></div>
          </div>
        </div>
      `}),h+="</div></div>",this.container.innerHTML=`
      ${h}
            ...
            <div style="display:flex; align-items:center; gap:0.75rem;">
              ...
              <div style="display:flex; align-items:center; gap:0.5rem; background:rgba(15,23,42,0.8); border:1px solid var(--border-color); padding:0.4rem 0.9rem; border-radius:20px;">
                ...
                <div style="width:48px; height:5px; background:rgba(255,255,255,0.1); border-radius:3px; overflow:hidden;">
                  <div id="question-timer-ring" style="width:${this.questionTimerSecondsRemaining/z.QUESTION_TIME_LIMIT_SEC*100}%; height:100%; background:${this.questionTimerSecondsRemaining<=15?"#ef4444":"var(--accent-cyan)"}; transition:width 0.9s linear;"></div>
                </div>
              </div>
              <!-- Pause Button -->
    ...
    `,this.attachEventListeners()}renderPlaygroundContent(e,i){const o=e.payload||{};if(e.type==="robot_mission")return`
        <div class="robot-mission-container">
          <div>
            ...
            <div class="blocks-palette">
              ${(o.availableBlocks||["Move Forward ⬆️","Turn Right ➡️","Grab Item 🦾"]).map(m=>`
                <button class="code-block" data-block="${m}">+ ${m}</button>
              `).join("")}
            </div>
          </div>
          <div>
            ...
            <div class="sequence-dropzone" id="sequence-box">
              ${i.robotSequence.length===0?'<span style="color:var(--text-secondary); font-size:0.85rem;">Click blocks on left to build sequence...</span>':i.robotSequence.map((m,p)=>`
                <div class="sequence-step" style="background:var(--accent-blue); padding:0.4rem 0.8rem; border-radius:6px; font-size:0.85rem; font-weight:600; display:flex; justify-content:space-between; align-items:center; gap:0.5rem;">
                  <span>📌 ${p+1}. ${m}</span>
                  <button class="remove-block-btn" data-idx="${p}" style="background:rgba(0,0,0,0.25); border:none; color:#fff; width:20px; height:20px; border-radius:50%; cursor:pointer; font-size:0.75rem; display:flex; align-items:center; justify-content:center; flex-shrink:0;">×</button>
                </div>
              `).join("")}
            </div>
          </div>
        </div>
      `;if(e.type==="picture_match"){const s=o.audioPromptText||e.instructions||"Select the matching item",m=o.options&&o.options.length>0?o.options:[{label:"Option A",emoji:"🤖"},{label:"Option B",emoji:"🍎"},{label:"Option C",emoji:"⚽"}];return`
        ...
        <div class="options-grid-3">
          ${m.slice(0,3).map((p,h)=>{const g=typeof p=="string"?p:p.label||`Option ${h+1}`,u=typeof p=="object"&&p.emoji?p.emoji:["🤖","🍎","⚽"][h]||"🎯";return`
              <button class="option-btn-3 ${i.selectedAnswerIndex===h?"selected":""}" data-opt="${h}">
                <span style="font-size: 2.2rem;">${u}</span>
                <span style="font-size: 0.95rem;">${g}</span>
              </button>
            `}).join("")}
        </div>
      `}if(e.type==="motor_target"){const s=o.targetsCount||3,m=i.motorClicks.length>=s;return`
        <div class="motor-canvas-container" id="motor-canvas">
          ${m?"":`<div class="motor-target" id="target-element" style="top: ${this.motorTargetPos.top}px; left: ${this.motorTargetPos.left}px;"></div>`}
          <div style="position:absolute; bottom:10px; left:15px; font-size:0.85rem; color:var(--text-secondary);">
            Targets Clicked: ${i.motorClicks.length} / ${s}
            ${m?" ✅ All targets hit!":""}
          </div>
        </div>
      `}const a=Array.isArray(o.options)&&o.options.length>0?o.options.slice(0,3):[{label:"Choice A"},{label:"Choice B"},{label:"Choice C"}],r=Array.isArray(o.sequence)?o.sequence:null;return`
      ${r?`
      ...
      `:""}
      <div class="options-grid-3">
        ${a.map((s,m)=>{const p=typeof s=="string"?s:s.label||s.text||JSON.stringify(s);return`
            <button class="option-btn-3 ${i.selectedAnswerIndex===m?"selected":""}" data-opt="${m}">
              <span>${p}</span>
            </button>
          `}).join("")}
      </div>
    `}registerInteraction(){if(!this.itemFirstInteractionTimestamp){this.itemFirstInteractionTimestamp=Date.now();const e=this.userAnswers[this.currentQuestionIndex];e&&(e.responseLatencyMs=Math.max(100,this.itemFirstInteractionTimestamp-this.itemStartTimestamp-this.pauseDurationForCurrentQuestionMs))}}attachEventListeners(){const e=this.userAnswers[this.currentQuestionIndex];this.container.querySelectorAll(".option-btn-3").forEach(S=>{S.addEventListener("click",c=>{this.registerInteraction();const w=c.currentTarget,f=parseInt(w.getAttribute("data-opt")||"0",10);e.selectedAnswerIndex=f,e.isSolved=!0,e.answeredAt=Date.now(),e.remainingTimeWhenAnsweredMs=this.questionTimerSecondsRemaining*1e3,e.attemptsCount=Math.max(1,e.attemptsCount+1),this.render()})}),this.container.querySelectorAll(".code-block").forEach(S=>{S.addEventListener("click",c=>{this.registerInteraction();const f=c.currentTarget.getAttribute("data-block");f&&(e.robotSequence.push(f),e.isSolved=e.robotSequence.length>0,e.answeredAt=Date.now(),e.attemptsCount=Math.max(1,e.attemptsCount+1),this.render())})}),this.container.querySelectorAll(".remove-block-btn").forEach(S=>{S.addEventListener("click",c=>{c.stopPropagation();const w=c.currentTarget,f=parseInt(w.getAttribute("data-idx")||"0",10);e.robotSequence.splice(f,1),e.isSolved=e.robotSequence.length>0,this.render()})});const r=this.container.querySelector("#clear-sequence-btn");r&&r.addEventListener("click",()=>{e.robotSequence=[],e.isSolved=!1,this.render()});const s=this.container.querySelector("#listen-audio-btn");s&&s.addEventListener("click",()=>{var c;const S=this.cachedActivities[this.currentQuestionIndex];if(S){const w=((c=S.payload)==null?void 0:c.audioPromptText)||S.instructions||"Select the matching item";this.speakAudio(w)}});const m=this.container.querySelector("#target-element"),p=this.container.querySelector("#motor-canvas");m&&p&&m.addEventListener("click",S=>{var D,b;this.registerInteraction();const c=m.getBoundingClientRect(),w=S.clientX-c.left,f=S.clientY-c.top,l=Math.sqrt(Math.pow(w-c.width/2,2)+Math.pow(f-c.height/2,2));e.motorClicks.push({x:w,y:f,dist:l}),e.attemptsCount=Math.max(1,e.attemptsCount+1);const d=p.getBoundingClientRect(),_=Math.floor(Math.random()*(d.height-80))+10,E=Math.floor(Math.random()*(d.width-80))+10;this.motorTargetPos={top:_,left:E};const F=((b=(D=this.cachedActivities[this.currentQuestionIndex])==null?void 0:D.payload)==null?void 0:b.targetsCount)||3;e.motorClicks.length>=F&&(e.selectedAnswerIndex=0,e.isSolved=!0,e.answeredAt=Date.now(),e.remainingTimeWhenAnsweredMs=this.questionTimerSecondsRemaining*1e3),this.render()});const h=this.container.querySelector("#pause-btn");h&&h.addEventListener("click",()=>this.pauseAssessment());const g=this.container.querySelector("#resume-btn");g&&g.addEventListener("click",()=>this.resumeAssessment());const u=this.container.querySelector("#submit-answer-btn");u&&u.addEventListener("click",()=>{this.isLoadingNextQuestion||(this.recordQuestionTimeData(e.isSolved),this.advanceToNextQuestion())})}showCompletionLoadingScreen(){this.container.innerHTML=`
    ...
    `}async completeAssessment(){var S,c,w,f;this.globalTimerInterval&&clearInterval(this.globalTimerInterval),this.questionTimerInterval&&clearInterval(this.questionTimerInterval),this.detachBeforeUnload(),z.clearSavedSession(),this.showCompletionLoadingScreen();const e=[],i={};for(let l=0;l<60;l++){const d=this.cachedActivities[l],_=this.userAnswers[l],E=U[l];if(!d)continue;let F=!1,D=0;if(d.type==="robot_mission"||Array.isArray((S=d.payload)==null?void 0:S.availableBlocks)){const b=((c=d.payload)==null?void 0:c.correctSequence)||[];let C=0;_.robotSequence.forEach((N,q)=>{b[q]===N&&C++}),D=b.length>0?C/b.length:_.robotSequence.length>0?1:0,F=D>=.8}else d.type==="motor_target"?(F=_.motorClicks.length>0,D=Math.min(1,_.motorClicks.length/(((w=d.payload)==null?void 0:w.targetsCount)||3))):(F=_.selectedAnswerIndex===(((f=d.payload)==null?void 0:f.correctIndex)??0),D=F?1:0);i[d.id]=E.maxPoints,e.push({item_id:d.id,domain:d.domain,skill:d.skill,difficulty_level:d.difficulty,is_correct:F,accuracy_score:D,response_time_ms:Math.max(1e3,_.timeSpentMs),expected_time_ms:9e4,attempts_count:Math.max(1,_.attemptsCount),hints_used:_.hintsUsed}),this.questionTimeRecords[l]&&(this.questionTimeRecords[l].earnedScore=oe.calculateItemScore(e[l],E.maxPoints))}const o=oe.calculateDomainScores(e,i),a=oe.calculateTotalScore(o),r=Ce.evaluatePlacement(a,o,e),s=["cognitive_ability","functional_skills","communication_level","behavioral_readiness","fine_motor_technology"],m={};s.forEach(l=>{const d=this.questionTimeRecords.filter(C=>C&&C.domain===l),_=d.reduce((C,N)=>C+(N.activeDurationMs||0),0),E=d.reduce((C,N)=>C+(N.pausedDurationMs||0),0),F=d.filter(C=>C.timedOut).length,D=d.map(C=>C.responseLatencyMs).filter(C=>C!==null),b=D.length>0?Math.round(D.reduce((C,N)=>C+N,0)/D.length):0;m[l]={totalActiveMs:_,totalPausedMs:E,questionsTimedOut:F,avgResponseLatencyMs:b}});const p=this.questionTimeRecords.reduce((l,d)=>l+((d==null?void 0:d.activeDurationMs)||0),0),h=this.breakEvents.reduce((l,d)=>l+d.breakDurationMs,0),g={session_id:`sess_60_${Date.now()}`,student_name:this.studentName||"Alex Rivers",age_group:"7-9",start_time:new Date(Date.now()-this.totalTimerSeconds*1e3).toISOString(),end_time:new Date().toISOString(),item_telemetries:e,domain_scores:o,total_score:a,placed_track:r.baseTrack,recommended_track:r.recommendedTrack,flags:r.flags.map(l=>l.id),question_time_records:this.questionTimeRecords,break_events:this.breakEvents,total_breaks_count:this.breakEvents.length,total_break_duration_ms:h,total_active_duration_ms:p,total_wall_clock_duration_ms:p+h,domain_time_summary:m},u=await this.analyzer.generateReportSummary(g,r);g.qualitative_summary=u,fe(this.container,g,r)}};z.STORAGE_KEY="cognix_active_assessment_session",z.QUESTION_TIME_LIMIT_SEC=90;let X=z,he=null;function ne(k="Alex Rivers",e=!0){const i=document.getElementById("app");i&&(he=new X(i),he.startSession(k,e))}function Fe(){X.clearSavedSession()}function ze(){const k=document.getElementById("app"),e=document.getElementById("childTestPage");k&&e&&(document.body.classList.add("exam-mode"),e.classList.remove("hidden"),e.classList.add("exam-active"),window.scrollTo(0,0),be(k))}window.initAssessment=ne;window.exitAssessment=Fe;window.openCEODashboard=ze;document.addEventListener("DOMContentLoaded",()=>{const k=document.getElementById("app"),e=document.getElementById("childTestPage"),i=X.getSavedSession();i&&e?(document.body.classList.add("exam-mode"),e.classList.remove("hidden"),e.classList.add("exam-active"),window.scrollTo(0,0),ne(i.studentName||"Alex Rivers",!0)):k&&!e&&ne("Alex Rivers",!1)});
